...
class InMemoryPersisterSpecification extends Specification with ThrownMessages {
  "in memory persister" should {
    "store events" in new ImMemoryPersisterSpecScope {
      ...
      persister ! StoreOne(Envelope("in4", 4, "xxxx".getBytes), move = false)
    ...
    "store 5 In's and 3 Out's and return 4 Envelopes on load query(4)" in new ImMemoryPersisterSpecScope {
      persister ! StoreOne(Envelope("in1", 4, "qwer".getBytes), move = false)
      persister ! Remove(List("inn1"))
      persister ! StoreOne(Envelope("in2", 4, "asdf".getBytes), move = false)
      persister ! Remove(List("inn2"))
      persister ! StoreOne(Envelope("in3", 4, "xzcv".getBytes), move = false)
      persister ! Remove(List("inn3"))
      persister ! StoreOne(Envelope("in4", 4, "tyui".getBytes), move = false)
      persister ! StoreOne(Envelope("in5", 4, "ghjk".getBytes), move = false)
